...
MODEL_TESTS_DIR_NAME = 'unittests'
REGRESSION_TESTS_DIR_NAME = 'unittests'
...
def get_test_models():
    ...
    for loc, dirpath in (MODEL_TESTS_DIR_NAME, MODEL_TEST_DIR), (REGRESSION_TESTS_DIR_NAME, REGRESSION_TEST_DIR), (CONTRIB_DIR_NAME, CONTRIB_DIR):
        for f in os.listdir(dirpath):
            if f.startswith('__init__') or f.startswith('.') or f.startswith('invalid'):
                continue
            models.append((loc, f))
    return models
...
#    failures = run_tests(test_labels, verbosity=verbosity, interactive=interactive, extra_tests=extra_tests)
    failures = run_tests(test_labels, verbosity=verbosity, extra_tests=extra_tests)
    if failures:
        sys.exit(failures)
